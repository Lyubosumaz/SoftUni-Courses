...
module.exports = {
    ...
    listAll: (req, res) => {
        Article.find().sort({ title: 'ascending' }).select('title')
            .then((articles) => {
    ...
    displayArticle: (req, res) => {
        ...
        Article.findById(articleId).populate('edits')
            .then((article) => {
                const sortedEdits = article.edits.sort((e1, e2) => {
                    return e2.createdDate - e1.createdDate;
                });
                article.newContent = sortedEdits[0].content.split('\r\n\r\n');
